...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("licenseRecog/classifications.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("licenseRecog/flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:                                         # for each char in plate
        ...
        cv2.rectangle(imgThreshColor, pt1, pt2, recognizer.SCALAR_GREEN, 2)           # draw green box around the char
